...
update  people 
set name='Adlar ddaha sonra eklenecek'
 where name is null;
...
update worker
set worker_salary = (select max(worker_salary) from (select * from worker)as w)*0.8
where worker_name='Veli Han';

-- Ali Can'ın maaşını en düşük maaştan yüzde 30 daha fazla düşürun
update worker
set worker_salary=(select min(worker_salary) from (select * from worker)as w)*1.3
