...
select ogr_ad, count(distinct(ogr_ad))isim_adedi
from ogrenci
group by ogr_ad;
-- Örnek 2: Sınıflardaki öğrenci sayısını bulun.
select sinif, count(ogr_ad) 
from ogrenci
group by sinif;
