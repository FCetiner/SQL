/*============================== JOIN İSLEMLERİ ===============================
    ...
    3) RIGHT JOIN: Sadece Ikinci sağ tabloda olan tum sonuclari gosterir
...
------------------------------------------------------------------------------*/ 
select sir.sirket_isim,sip.sirket_id,sip.siparis_tarihi,sip.sirket_id -- joinlerde hangi tablodan alacagımızı soylememiz gerekebilir
from sirketler sir
...
-- bağlama işi joinlerde on keywordu ile yapılır
...
from bolumler b left join personel p
on p.bolum_id= b.bolum_id 
where bolum_isim in('satis','mudurluk') and p.maas>2000
order by b.bolum_isim, p.personel_isim;
...
------------------------------------------------------------------------------*/ 
select b.bolum_isim, p.personel_isim,p.maas, (select personel_isim from personel where p.mudur_id=personel_id)mudur_isim
