...
update calisanlar
set maas=maas*1.2
where maas <(select round(avg(maas)) from (select * from calisanlar)ortalama_maas);
select * from calisanlar;


-- 4-) Çalışanların isim ve cocuk_sayisi'ni listeleyen bir sorgu yazınız. 
select isim, (select cocuk_sayisi from aileler where calisanlar.id=aileler.id)cocuk_sayisi from  calisanlar;
